# IPFS核心节点公网连接检查工具

## 🎯 快速使用

### 前提条件

1. 确保已安装必要工具：
```bash
# 检查curl
which curl

# 检查jq（JSON处理工具）
which jq

# 如果jq未安装，执行：
sudo apt-get install jq -y
```

2. 确保IPFS节点正在运行：
```bash
# 检查IPFS进程
ps aux | grep ipfs

# 或检查systemd服务
systemctl status ipfs
```

### 执行检查

```bash
# 进入项目目录
cd /home/xiaodong/文档/stardust

# 运行检查脚本
./scripts/check-ipfs-public-network.sh
```

### 预期输出

```
========================================
IPFS核心节点公网连接检查
========================================

========================================
检查节点 1/3: Core Node 1
========================================

检查节点是否在线: Core Node 1 (http://localhost:5001)
✓ 节点在线

检查节点信息: Core Node 1
Peer ID: QmXXXXXXXXXXXXXXXX...

监听地址:
/ip4/127.0.0.1/tcp/4001
/ip4/192.168.1.100/tcp/4001
/ip4/203.0.113.10/tcp/4001
✓ 检测到公网地址: /ip4/203.0.113.10/tcp/4001

检查Swarm对等节点: Core Node 1
总对等节点数: 156
  - 公网节点: 143
  - 私有节点: 13
✓ 连接到足够的公网节点 (143个)

检查DHT功能: Core Node 1
测试DHT查询 (CID: QmYwAPJzv5CZsnA625s3Xf2nemtYgPpHdWEz79ojWnPbdG)...
✓ DHT查询成功 - 找到 8 个提供者

检查IPFS配置: Core Node 1
检查Routing配置...
✓ Routing类型: dht (DHT已启用)

========================================
检查完成
========================================

✓ 所有核心节点均已正确连接到IPFS公网 ✓

报告已保存到: ipfs-public-network-check-20251027-143025.md

========================================
报告位置
========================================
完整路径: /home/xiaodong/文档/stardust/ipfs-public-network-check-20251027-143025.md
```

### 查看报告

```bash
# 查看最新的检查报告
cat ipfs-public-network-check-*.md | tail -100

# 或使用文本编辑器
nano ipfs-public-network-check-*.md
```

---

## 🔧 配置说明

### 自定义检查的节点

编辑脚本开头的配置部分：

```bash
nano scripts/check-ipfs-public-network.sh
```

找到并修改：
```bash
# 配置：3个核心节点的API地址
NODES=(
    "http://localhost:5001"      # 节点1
    "http://localhost:5002"      # 节点2
    "http://localhost:5003"      # 节点3
)

# 节点名称
NODE_NAMES=(
    "Core Node 1"
    "Core Node 2"
    "Core Node 3"
)
```

### 如果只有1个节点

```bash
NODES=(
    "http://localhost:5001"
)

NODE_NAMES=(
    "Main IPFS Node"
)
```

### 如果节点在远程服务器

```bash
NODES=(
    "http://192.168.1.100:5001"
    "http://192.168.1.101:5001"
    "http://192.168.1.102:5001"
)
```

**注意**: 确保防火墙允许访问远程IPFS API端口（默认5001）。

---

## 📊 检查结果判断

### ✅ 理想结果（推荐）

所有指标都应该是：
- ✓ 节点在线
- ✓ 公网对等节点 > 50个
- ✓ DHT功能正常
- ✓ Routing配置为 `dht`

### ⚠️ 需要优化

如果看到：
- ⚠️ 公网对等节点 < 10个
- ⚠️ DHT查询超时
- ⚠️ Routing配置为 `dhtclient`

**操作**: 参考检查指南进行优化

### ❌ 需要立即修复

如果看到：
- ❌ 节点离线
- ❌ 公网对等节点 = 0
- ❌ DHT查询失败
- ❌ Routing配置为 `none`

**操作**: 按照检查指南紧急修复步骤操作

---

## 🔍 常见检查场景

### 场景1：新部署的IPFS节点

首次检查可能看到较少的公网连接，这是正常的。等待5-10分钟后再次检查：

```bash
# 等待节点发现更多peers
sleep 600  # 10分钟

# 再次检查
./scripts/check-ipfs-public-network.sh
```

### 场景2：从私有网络迁移到公网

如果节点之前配置为私有网络，需要：

```bash
# 1. 清理旧的配置
ipfs config --json Swarm.AddrFilters '[]'

# 2. 启用公网DHT
ipfs config Routing.Type dht

# 3. 添加公网引导节点
ipfs bootstrap add --default

# 4. 重启IPFS
ipfs shutdown
ipfs daemon &

# 5. 等待5分钟后检查
sleep 300
./scripts/check-ipfs-public-network.sh
```

### 场景3：检查特定功能

如果只想检查某个具体功能：

```bash
# 只检查对等节点数
curl -X POST "http://localhost:5001/api/v0/swarm/peers" | jq '.Peers | length'

# 只检查DHT功能
curl -X POST "http://localhost:5001/api/v0/dht/findprovs?arg=QmYwAPJzv5CZsnA625s3Xf2nemtYgPpHdWEz79ojWnPbdG"

# 只检查Routing配置
curl -X POST "http://localhost:5001/api/v0/config?arg=Routing.Type" | jq -r '.Value'
```

---

## 🐛 故障排查

### 问题1：脚本报错 "jq: command not found"

**解决**:
```bash
sudo apt-get update
sudo apt-get install jq -y
```

### 问题2：脚本报错 "curl: (7) Failed to connect"

**原因**: IPFS节点未运行或端口错误

**解决**:
```bash
# 检查IPFS是否运行
ipfs id

# 如果未运行，启动IPFS
ipfs daemon &

# 检查API端口
ipfs config Addresses.API
```

### 问题3：所有节点显示"离线"

**解决**:
```bash
# 1. 检查IPFS进程
ps aux | grep ipfs

# 2. 如果没有进程，启动IPFS
ipfs daemon &

# 3. 等待启动完成（约30秒）
sleep 30

# 4. 再次运行检查
./scripts/check-ipfs-public-network.sh
```

### 问题4：权限错误

**解决**:
```bash
# 确保脚本有执行权限
chmod +x scripts/check-ipfs-public-network.sh

# 如果需要访问远程节点，确保网络可达
ping <远程节点IP>
telnet <远程节点IP> 5001
```

---

## 📅 定期检查建议

### 设置自动检查（推荐）

```bash
# 1. 编辑crontab
crontab -e

# 2. 添加每日检查（每天凌晨2点）
0 2 * * * cd /home/xiaodong/文档/stardust && ./scripts/check-ipfs-public-network.sh >> logs/ipfs-check.log 2>&1

# 3. 添加每周检查（每周一上午9点）
0 9 * * 1 cd /home/xiaodong/文档/stardust && ./scripts/check-ipfs-public-network.sh && mail -s "IPFS Weekly Check" admin@example.com < ipfs-public-network-check-*.md

# 4. 保存并退出
```

### 手动定期检查

建议频率：
- **新部署**: 每小时检查一次（前24小时）
- **稳定运行**: 每天检查一次
- **生产环境**: 每周检查一次 + 监控告警

---

## 📖 相关文档

- [IPFS核心节点公网连接检查指南](../docs/IPFS核心节点公网连接检查指南.md)
- [stardust-ipfs三需求分析报告](../docs/stardust-ipfs三需求分析报告.md)
- [IPFS官方文档 - Configuration](https://docs.ipfs.tech/how-to/configure-node/)
- [IPFS官方文档 - DHT](https://docs.ipfs.tech/concepts/dht/)

---

## 🆘 获取帮助

如果检查后仍有问题：

1. **查看详细报告**: 检查生成的Markdown报告文件
2. **收集诊断信息**:
   ```bash
   # IPFS配置
   ipfs config show > ipfs-config.json
   
   # IPFS日志
   ipfs log tail > ipfs-log.txt
   
   # 网络状态
   netstat -tulpn | grep ipfs > network-status.txt
   ```

3. **提交问题**:
   - 附上检查报告
   - 附上诊断信息
   - 描述网络环境（公网/内网/NAT等）

---

**工具版本**: v1.0  
**最后更新**: 2025-10-27

