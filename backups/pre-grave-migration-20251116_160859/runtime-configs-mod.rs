...
impl pallet_deceased::Config for Runtime {
    ...
    /// 暂时使用Dummy实现，实际应该连接到 pallet-pricing
    type PricingProvider = DummyPricingProvider;
...
/// 函数级中文注释：Dummy Pricing Provider实现（临时）
/// TODO: 实际应该使用 pallet-pricing 提供真实汇率
pub struct DummyPricingProvider;
impl pallet_deceased::governance::PricingProvider for DummyPricingProvider {
    fn get_current_exchange_rate() -> Result<u64, &'static str> {
        // 返回固定汇率：1 DUST = 0.5 USDT (500000 = 0.5 * 1e6)
        Ok(500000u64)
    }
}
...
/// 函数级详细中文注释：Memorial TargetControl 实现（方案A - 仅支持Grave）
///
/// ### 破坏式变更
/// - 仅处理 domain=0（Grave）
/// - 其他域返回 false，为未来扩展预留
...
impl pallet_memorial::TargetControl<RuntimeOrigin, AccountId> for MemorialTargetControl {
    ...
    fn ensure_allowed(origin: RuntimeOrigin, grave_id: u64) -> frame_support::dispatch::DispatchResult {
        let who = frame_system::ensure_signed(origin)?;

        // 检查墓位是否存在且公开
        let grave = pallet_stardust_grave::Graves::<Runtime>::get(grave_id)
            .ok_or(sp_runtime::DispatchError::from("GraveNotFound"))?;

        // 检查墓位是否公开或用户是否有权限
        if !grave.is_public {
            // 如果不公开，检查是否是墓主或管理员
            let is_owner = grave.owner == who;
            let admins = pallet_stardust_grave::GraveAdmins::<Runtime>::get(grave_id);
            let is_admin = admins.contains(&who);

            if !is_owner && !is_admin {
                return Err(sp_runtime::DispatchError::from("NotAllowed"));
            }
        }
...
/// 函数级详细中文注释：墓地所有者查询适配器（P0修复）
/// - 从pallet-stardust-grave读取墓地所有者信息
/// - 用于供奉分账，将目标分成转给墓地所有者
pub struct MemorialGraveProvider;
impl pallet_memorial::GraveProvider<AccountId> for MemorialGraveProvider {
    fn owner_of(grave_id: u64) -> Option<AccountId> {
        pallet_stardust_grave::Graves::<Runtime>::get(grave_id).map(|g| g.owner)
    }
}
...
/// - 查询墓位关联的所有逝者
/// - 检查用户是否已是亲友团成员
/// - 如果不是，自动加入为 Member 角色
/// - 发送 AutoJoinedFriend 事件
///
/// ### 2. 🆕 Affiliate分账处理（新增功能）
/// - 触发affiliate联盟分账系统
/// - 支持15层推荐链分账
/// - 100%资金进入推荐链分账
/// - 统一购买和续费的分账逻辑
...
impl pallet_memorial::Config for Runtime {
    ...
    /// 函数级中文注释：墓地所有者查询接口
    type GraveProvider = MemorialGraveProvider;
...
impl pallet_arbitration::pallet::ArbitrationRouter<AccountId, Balance> for ArbitrationRouter {
    ...
    fn get_counterparty(
        ...
        if domain == OtcOrderNsBytes::get() {
            ...
            // TODO: SimpleBridge 需要实现对方账户查询
            // 暂时返回错误，需要根据实际的 Bridge pallet 存储结构实现
            Err(sp_runtime::DispatchError::Other("NotImplementedForBridge"))
        } else {
            Err(sp_runtime::DispatchError::Other("UnsupportedDomain"))
        }
    }
    ...
    fn get_order_amount(domain: [u8; 8], id: u64) -> Result<Balance, sp_runtime::DispatchError> {
        if domain == OtcOrderNsBytes::get() {
            ...
            // SimpleBridge：从兑换信息中获取 DUST 金额
            // TODO: 实现 Bridge 金额查询
            // let swap = pallet_bridge::Swaps::<Runtime>::get(id)
            //     .ok_or(sp_runtime::DispatchError::Other("SwapNotFound"))?;
            // Ok(swap.dust_amount)
            Err(sp_runtime::DispatchError::Other("NotImplementedForBridge"))
        } else {
            Err(sp_runtime::DispatchError::Other("UnsupportedDomain"))
...
impl pallet_deceased_ai::DeceasedDataProvider<u64, AccountId> for DeceasedAIDataAdapter {
    ...
    fn get_deceased_works(
        ...
        // 当前返回空列表，待 pallet-deceased 实现作品管理后完善
        Ok((Vec::new(), 0))
    }

    fn get_work_details(_work_id: u64) -> Result<pallet_deceased_ai::ExportedWork, sp_runtime::DispatchError> {
        // TODO: 实现从 pallet-deceased 获取作品详情
        // 当前返回错误，待 pallet-deceased 实现作品管理后完善
        Err(sp_runtime::DispatchError::Other("Work management not yet implemented"))
    }

    fn get_ai_training_works(_deceased_id: u64) -> Result<Vec<u64>, sp_runtime::DispatchError> {
        // TODO: 实现从 pallet-deceased 获取AI训练作品列表
        // 当前返回空列表，待 pallet-deceased 实现作品管理后完善
        Ok(Vec::new())
    }
}
