...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_deceased(
            ...
            // 建立owner索引
            OwnerDepositsByOwner::<T>::insert((who.clone(), deceased_id_u64), ());
        ...
        pub fn transfer_deceased_owner(
            ...
            // 10. 更新 OwnerDepositsByOwner 索引
            OwnerDepositsByOwner::<T>::remove((who.clone(), deceased_id_u64));
            OwnerDepositsByOwner::<T>::insert((new_owner.clone(), deceased_id_u64), ());
        ...
        pub fn delete_album(
            ...
            if let Some(photos) = PhotosByAlbum::<T>::get(album_id) {
        ...
        pub fn complain_text(
            ...
            let reason_bounded: BoundedVec<u8, ConstU32<1024>> = reason.try_into()
                .map_err(|_| Error::<T>::BadInput)?;
    ...
    impl<T: Config> Pallet<T> {
        /// 函数级详细中文注释：内部检查押金是否充足
        ///
        /// ### 功能
        /// - 检查逝者拥有者的押金余额是否达到最低要求
        /// - 不锁定任何押金，仅做检查
        ///
        /// ### 参数
        /// - `deceased_id`: 逝者ID
        ...
        pub fn ensure_sufficient_deposit_internal(deceased_id: u64) -> DispatchResult {
            ...
            let min_required_usdt = 2u32; // TODO: 改为配置项
                ...
                deposit_record.available_usdt >= min_required_usdt,
        ...
        fn is_video_collection_under_complaint(collection_id: T::VideoCollectionId) -> bool {
            // 获取视频集下的所有视频
            if let Some(videos) = VideosByCollection::<T>::get(collection_id) {
