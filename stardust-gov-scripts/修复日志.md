...
**失败详情**:
- 转账金额: 0.000042 MEMO、0.000091 MEMO、0.000037 MEMO、0.000027 MEMO
- 成功笔数: 0
- 失败笔数: 4
...
- 换算为 MEMO: `10_000_000 / 10^12 = 0.00001 MEMO`
- 链上最小转账金额: 通常为 0.01 MEMO 到 1 MEMO
- 生成的随机金额范围: 0.00001 ~ 0.0001 MEMO (远低于最小限额)
...
```javascript
// 转账金额最小值（最小单位）
// 1 MEMO = 1_000_000_000_000 (精度12)
// 设置为 1 MEMO 作为最小值，生成 1-10 MEMO 的随机金额
const MIN_AMOUNT = 1_000_000_000_000n; // 1 MEMO
```

**改进倍数**: 100,000 倍 (从 0.00001 MEMO 提升到 1 MEMO)
...
修复后，批量转账将生成 1-10 MEMO 的随机金额，确保：
1. ✅ 高于链上最小转账限额
2. ✅ 金额范围合理（1-10 MEMO）
3. ✅ 符合实际使用场景
...
```javascript
// 设置更小的金额（如 0.1 MEMO）
const MIN_AMOUNT = 100_000_000_000n; // 0.1 MEMO

// 设置更大的金额（如 10 MEMO）
const MIN_AMOUNT = 10_000_000_000_000n; // 10 MEMO

// 设置更大的金额（如 100 MEMO）
const MIN_AMOUNT = 100_000_000_000_000n; // 100 MEMO
```
...
例如:
1 MEMO = 1,000,000,000,000 最小单位
0.1 MEMO = 100,000,000,000 最小单位
10 MEMO = 10,000,000,000,000 最小单位
```
...
此次修复解决了因转账金额过小导致的批量转账失败问题。修改后的脚本将生成合理范围的随机金额（1-10 MEMO），确保转账能够成功执行。
