...
**当前配置**:
```javascript
MIN_AMOUNT = 1_000_000_000_000_000_000_000n  // 1,000,000,000 DUST (10亿)
```
- **金额范围**: 10亿 ~ 100亿 DUST
- **转账对象**: 4个预设地址

**修复历史**:
- v1.0.1: 修复 `BelowMinimum` 错误（金额从 0.00001 提升到 1 DUST）
- v1.0.2: 升级为大额转账配置（1 DUST → 10亿 DUST）
...
- **原因**: 金额 0.00001 DUST 低于最小限额
- **修复**: 提升到 1 DUST（v1.0.1）
- **当前**: 10亿 DUST（v1.0.2）
...
### 批量转账
1. **超大金额**: 当前配置为 10亿-100亿 DUST/笔
2. **余额要求**: 发送账户需要 ≥200亿 DUST
3. **测试建议**: 强烈建议先在测试网络验证
4. **不可撤销**: 区块链转账不可撤销
