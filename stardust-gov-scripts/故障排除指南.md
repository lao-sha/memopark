# create-offerings.js 故障排除指南

**问题**: 运行脚本时闪退  
**诊断工具**: `test-connection.js`  
**最后更新**: 2025-10-27

---

## 🔍 快速诊断

### 运行诊断脚本

```bash
cd /home/xiaodong/文档/stardust/stardust-gov-scripts
node test-connection.js
```

诊断脚本会检查：
1. ✅ Node.js 版本
2. ✅ 依赖安装
3. ✅ 链节点连接
4. ✅ Pallet 可用性

---

## ❌ 常见问题

### 问题1: 链节点未启动 ⭐⭐⭐（最常见）

**症状**:
```
❌ 连接超时（5秒）
💡 可能的原因:
   1. 链节点未启动
```

**解决方案**:

#### 步骤1: 检查节点是否运行

```bash
# 检查节点进程
ps aux | grep node-template

# 检查端口
netstat -tuln | grep 9944
# 或
lsof -i :9944
```

#### 步骤2: 启动链节点

**开发模式（推荐）**:
```bash
cd /home/xiaodong/文档/stardust

# 清理旧数据（可选）
rm -rf my-chain-state/chains/dev

# 启动开发模式
./target/release/node-template --dev

# 或使用脚本（如果有）
./启动所有服务.sh
```

**生产模式**:
```bash
cd /home/xiaodong/文档/stardust

# 启动节点
./target/release/node-template \
  --base-path ./my-chain-state \
  --chain local \
  --alice \
  --port 30333 \
  --rpc-port 9933 \
  --ws-port 9944 \
  --rpc-cors all
```

#### 步骤3: 等待节点就绪

```bash
# 等待10秒
sleep 10

# 测试连接
node test-connection.js
```

**成功标志**:
```
✅ 已连接 Stardust Testnet • Stardust Node v1.0.0
✅ memoOfferings pallet 可用
✅ createOffering extrinsic 可用
```

---

### 问题2: 依赖未安装

**症状**:
```
❌ 依赖缺失: Cannot find module '@polkadot/api'
```

**解决方案**:

```bash
cd /home/xiaodong/文档/stardust/stardust-gov-scripts

# 安装依赖
npm install

# 验证安装
npm list @polkadot/api
npm list @polkadot/util-crypto
```

**如果安装失败**:

```bash
# 清理缓存
rm -rf node_modules package-lock.json
npm cache clean --force

# 重新安装
npm install

# 或使用 yarn
yarn install
```

---

### 问题3: WS 地址错误

**症状**:
```
节点地址: ws://192.168.1.100:9944
❌ 连接超时（5秒）
```

**解决方案**:

```bash
# 检查节点实际监听的地址
netstat -tuln | grep 9944

# 如果节点在本地
export STARDUST_WS=ws://127.0.0.1:9944

# 如果节点在远程服务器
export STARDUST_WS=ws://192.168.1.100:9944

# 运行脚本
npm run create-offerings
```

---

### 问题4: 端口被占用

**症状**:
```
Error: bind EADDRINUSE 0.0.0.0:9944
```

**解决方案**:

```bash
# 查找占用端口的进程
lsof -i :9944

# 或
netstat -tuln | grep 9944

# 杀死进程（替换 PID）
kill -9 <PID>

# 或使用不同端口启动节点
./target/release/node-template --dev --ws-port 9945

# 更新环境变量
export STARDUST_WS=ws://127.0.0.1:9945
```

---

### 问题5: memo-offerings pallet 不可用

**症状**:
```
✅ 已连接 Stardust Testnet
❌ memoOfferings pallet 不可用
```

**解决方案**:

#### 检查 runtime 配置

```bash
cd /home/xiaodong/文档/stardust

# 检查 runtime 中是否包含 memo-offerings
grep -r "memo_offerings" runtime/src/
```

#### 重新编译（如果 pallet 缺失）

```bash
cd /home/xiaodong/文档/stardust

# 确认 Cargo.toml 中已添加 memo-offerings
cat runtime/Cargo.toml | grep memo-offerings

# 重新编译
cargo build --release

# 清理旧链数据
rm -rf my-chain-state/chains/dev

# 重新启动
./target/release/node-template --dev
```

---

### 问题6: 权限不足

**症状**:
```
❌ 创建失败: system.BadOrigin: Bad origin
```

**解决方案**:

检查管理员账户配置：

```javascript
// create-offerings.js
const ADMIN_CONFIG = {
  mnemonic: 'your admin mnemonic here',  // ← 确保是管理员账户
  expectedAddress: '5C7R...',
};
```

**验证管理员权限**:

```bash
# 使用 polkadot.js Apps
# 1. 访问 https://polkadot.js.org/apps
# 2. 连接到节点
# 3. Developer → Chain state
# 4. 选择 sudo → key()
# 5. 确认返回的地址与 ADMIN_CONFIG.expectedAddress 一致
```

---

### 问题7: 余额不足

**症状**:
```
❌ 余额不足！
   可用: 0.01 MEMO
   需要: 0.5 MEMO
```

**解决方案**:

```bash
# 方法1: 从水龙头获取测试代币（如果有）
# 访问测试网水龙头

# 方法2: 从其他账户转账
# 使用 batch-transfer.js 或手动转账

# 方法3: 使用富账户（开发模式）
# Alice: 5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY
# Bob: 5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty
```

---

## 🛠️ 调试技巧

### 1. 启用详细日志

在脚本中添加：

```javascript
// create-offerings.js 开头添加
process.env.DEBUG = '*';

// 或
console.log('DEBUG: 配置详情');
console.log(JSON.stringify(CREATE_CONFIG, null, 2));
```

---

### 2. 单步调试

```javascript
// 只创建1个供奉品测试
const CREATE_CONFIG = {
  offeringCount: 1,
  // ...
};
```

---

### 3. 使用 try-catch 捕获更多信息

```javascript
try {
  await createOffering(...);
} catch (error) {
  console.error('详细错误:', error);
  console.error('错误类型:', error.constructor.name);
  console.error('错误堆栈:', error.stack);
}
```

---

### 4. 检查链日志

```bash
# 查看节点输出
tail -f /path/to/node.log

# 或直接在终端运行节点查看实时日志
./target/release/node-template --dev
```

---

## 📋 完整故障排除流程

### 步骤1: 运行诊断

```bash
cd /home/xiaodong/文档/stardust/stardust-gov-scripts
node test-connection.js
```

---

### 步骤2: 根据诊断结果处理

**如果显示连接超时**:
→ 启动链节点

**如果显示依赖缺失**:
→ `npm install`

**如果显示 pallet 不可用**:
→ 检查 runtime 配置并重新编译

---

### 步骤3: 再次运行诊断

```bash
node test-connection.js
```

---

### 步骤4: 运行脚本

```bash
npm run create-offerings
```

---

## 🔍 高级诊断

### 检查节点 RPC

```bash
# 使用 curl 测试
curl -H "Content-Type: application/json" \
     -d '{"id":1, "jsonrpc":"2.0", "method": "system_chain"}' \
     http://127.0.0.1:9933

# 应该返回
{"jsonrpc":"2.0","result":"Stardust Testnet","id":1}
```

---

### 检查 WebSocket

```bash
# 安装 wscat
npm install -g wscat

# 测试 WebSocket
wscat -c ws://127.0.0.1:9944

# 发送 RPC 请求
> {"id":1, "jsonrpc":"2.0", "method": "system_chain"}
```

---

### 查看链元数据

```javascript
// test-metadata.js
const { ApiPromise, WsProvider } = require('@polkadot/api');

async function checkMetadata() {
  const api = await ApiPromise.create({ 
    provider: new WsProvider('ws://127.0.0.1:9944') 
  });
  
  const metadata = await api.rpc.state.getMetadata();
  const pallets = metadata.asLatest.pallets;
  
  console.log('可用的 Pallets:');
  pallets.forEach(p => console.log(`  - ${p.name.toString()}`));
  
  await api.disconnect();
}

checkMetadata();
```

---

## 📞 仍然无法解决？

### 收集信息

```bash
# 1. Node.js 版本
node --version

# 2. npm 版本
npm --version

# 3. 依赖版本
npm list @polkadot/api
npm list @polkadot/util-crypto

# 4. 系统信息
uname -a

# 5. 节点进程
ps aux | grep node-template

# 6. 端口状态
netstat -tuln | grep 9944

# 7. 诊断结果
node test-connection.js > diagnostic.log 2>&1
```

### 提供以下信息

1. 诊断脚本输出 (`diagnostic.log`)
2. 节点日志
3. 错误截图
4. 运行环境（开发/测试/生产）

---

## 🎯 快速参考

| 问题 | 快速解决 |
|------|---------|
| 连接超时 | 启动节点：`./target/release/node-template --dev` |
| 依赖缺失 | `npm install` |
| 端口占用 | `lsof -i :9944` 然后 `kill -9 <PID>` |
| 权限不足 | 使用管理员账户 |
| 余额不足 | 转入 MEMO 代币 |
| Pallet不可用 | 重新编译并清理链数据 |

---

## 🚀 成功标志

运行 `node test-connection.js` 应该看到：

```
✅ 诊断完成 - 所有检查通过！
💡 可以运行: npm run create-offerings
```

然后运行脚本：

```
npm run create-offerings
```

应该看到：

```
🚀 创建供奉品脚本启动
✅ 加密库准备完成
✅ 管理员账户地址验证通过
✅ 已连接 Stardust Testnet
💰 可用余额: 1000000.0 MEMO
...
```

---

**文档版本**: 1.0.0  
**最后更新**: 2025-10-27  
**适用脚本**: create-offerings.js v1.0.0

