...
- **Substrate SDK**: @polkadot/api + @polkadot/extension-dapp（或 Dedot 可选）
...
# Substrate SDK（默认）
pnpm i @polkadot/api @polkadot/extension-dapp
...
export async function getInjectedAccounts() {
  ...
  return web3Accounts(); // [{ address, meta }, ...]
}
```

---

### 示例页面：连接账户 + 发送一次 remark 交易

```tsx
// src/App.tsx
import React from 'react';
import { Button, Card, Space, Select, Typography, message } from 'antd';
import { getApi } from './shared/polkadot';
import { getInjectedAccounts } from './shared/accounts';
...
export default function App() {
  ...
  React.useEffect(() => {
    getApi().then(() => setApiReady(true)).catch(() => setApiReady(false));
  }, []);

  const loadAccounts = async () => {
    const list = await getInjectedAccounts();
    setAccounts(list);
    if (list.length > 0) setAddr(list[0].address);
  };

  const sendRemark = async () => {
    if (!addr) return message.warning('请选择账户');
    setLoading(true);
    try {
      const api = await getApi();
      const injector = await web3FromAddress(addr);
      const tx = api.tx.system.remark('hello-memopark');
      await tx.signAndSend(
        addr,
        { signer: injector.signer },
        ({ status, dispatchError }) => {
          if (dispatchError) {
            message.error(dispatchError.toString());
            setLoading(false);
          }
          if (status.isInBlock) {
            message.success(`已上链: ${status.asInBlock.toString()}`);
            setLoading(false);
          }
        }
      );
  ...
  return (
    <div style={{ padding: 16 }} className="safe-area">
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        <Card title="连接链" bordered>
          <Space>
            <Text type={apiReady ? 'success' : 'secondary'}>
              {apiReady ? '已连接 WS' : '未连接'}
            </Text>
            <Button onClick={loadAccounts}>加载钱包账户</Button>
          </Space>
        </Card>

        <Card title="账户" bordered>
          <Space direction="vertical" style={{ width: '100%' }}>
            <Select
              value={addr}
              placeholder="选择账户"
              options={accounts.map(a => ({ value: a.address, label: a.meta.name || a.address }))}
              onChange={setAddr}
              style={{ width: '100%' }}
            />
            <Text type="secondary">推荐用移动钱包内置 DApp 浏览器打开。</Text>
          </Space>
        </Card>

        <Card title="示例操作" bordered>
          <Space>
            <Button type="primary" onClick={sendRemark} loading={loading} disabled={!addr}>
              发送 remark
            </Button>
          </Space>
        </Card>
