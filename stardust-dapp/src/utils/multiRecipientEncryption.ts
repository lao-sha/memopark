...
export class MultiRecipientEncryption {
   ...
   * 加密数据给多个接收方
   * 
   * @param content - 原始内容（JSON对象或字符串）
   * @param recipientPublicKeys - 接收方公钥映射 { accountId: publicKey }
   * @param encryptorAccountId - 加密者账户ID（可选）
   * @param description - 内容描述（可选）
   * @returns 多接收方加密数据结构
   * 
   * @example
   * ```typescript
   * const publicKeys = {
   *   '5GrwvaEF...': decodeAddress('5GrwvaEF...'),
   *   '5FHneW46...': decodeAddress('5FHneW46...'),
   * };
   * 
   ...
   * @param recipientAccountId - 当前用户账户ID
   * @param recipientSecretKey - 当前用户私钥（32字节）
   * @returns 解密后的原始内容
   * 
   * @throws {Error} 当用户无权解密或解密失败时抛出错误
   * 
   * @example
   * ```typescript
