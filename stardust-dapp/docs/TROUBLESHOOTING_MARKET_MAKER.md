...
# 手动创建第一个申请（使用 sudo）
> api.tx.sudo.sudo(
    api.tx.marketMaker.lockDeposit('1000000000000000')  # 1000 DUST
  ).signAndSend(alice)
```
...
**示例**：
```typescript
formatMemoAmount(1000)     // "1000000000000000" (1000 DUST)
formatMemoAmount(0.001)    // "1000000000" (0.001 DUST)
formatMemoAmount(100.5)    // "100500000000000" (100.5 DUST)
```
