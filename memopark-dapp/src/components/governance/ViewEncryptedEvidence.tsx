...
export const ViewEncryptedEvidence: React.FC<ViewEncryptedEvidenceProps> = ({
  ...
  const handleDecrypt = async () => {
    if (!encryptedData || !currentAccount || !injector) {
    ...
    try {
      ...
      const privateKey = await getPrivateKeyFromWallet(currentAccount.address, injector);
  ...
  const logEvidenceAccess = async (orderId: number, evidenceCid: string) => {
    if (!api || !currentAccount || !injector) return;
    
    try {
      // 调用链上接口记录访问（pallet-evidence 或自定义）
      // 这里是示例代码
      console.log(`📝 记录访问日志: 委员 ${currentAccount.address} 查看了订单 ${orderId} 的证据`);
