...
mod benches {
	...
	fn commit() {
		...
		let domain: u8 = 1;
		let target: u64 = 1;
		let make_cid = || -> BoundedVec<u8, T::MaxCidLen> { BoundedVec::try_from(Vec::from("bafy...".as_bytes())).unwrap() };
		let imgs = vec![make_cid(); 2];
		let vids = vec![];
		let docs = vec![];
		let memo: Option<BoundedVec<u8, T::MaxMemoLen>> = None;
		#[extrinsic_call]
		_(RawOrigin::Signed(caller), domain, target, imgs, vids, docs, memo);
	}
	...
	fn link() {
		...
		let memo: Option<BoundedVec<u8, T::MaxMemoLen>> = None;
		Pallet::<T>::commit_hash(RawOrigin::Signed(caller.clone()).into(), ns, subject, hash_h256, memo).unwrap();
