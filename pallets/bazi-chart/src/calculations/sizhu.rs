...
/// - `None`: 参数无效或计算失败
pub fn calculate_month_ganzhi(_year: u16, month: u8, day: u8, year_gan: u8) -> Option<GanZhi> {
	// 参数验证
	if month < 1 || month > 12 || day < 1 || day > 31 || year_gan >= 10 {
		return None;
	}
	...
	// 简化实现：根据公历月份近似判断
	// 实际应使用精确节气计算
	let month_zhi = match month {
		1 => {
			// 1月：小寒到立春之间
			// 小寒一般在1月5-7日，立春在2月3-5日
			if day < 6 {
				0  // 子月（上月延续）
			} else {
				1  // 丑月
