...
pub mod pallet {
    ...
    use codec::Decode;
    use frame_support::{
        dispatch::GetDispatchInfo, pallet_prelude::*, traits::Contains, BoundedVec,
    };
    ...
    pub trait Config: frame_system::Config {
        ...
        type PermitSignature: Verify<Signer = Self::PermitSigner>
            + codec::Decode
            + codec::MaxEncodedLen
            + TypeInfo
            + Parameter;
        type PermitSigner: IdentifyAccount<AccountId = Self::AccountId>
            + codec::Decode
            + TypeInfo
            + Parameter
            + Clone;
    ...
    pub type Sessions<T: Config> = StorageDoubleMap<
        ...
        Blake2_128Concat,
        [u8; 16],
        SessionPermit<T::AccountId, BlockNumberFor<T>>,
        OptionQuery,
    >;

    /// 函数级中文注释：每块代付转发条数统计（用于全局节流）。
    #[pallet::storage]
    pub type ForwardedPerBlock<T: Config> =
        StorageMap<_, Blake2_128Concat, BlockNumberFor<T>, u32, ValueQuery>;

    /// 函数级中文注释：赞助者×命名空间的窗口统计 (window_start, calls, ref_time)。
    #[pallet::storage]
    pub type SponsorWindowStats<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat,
        T::AccountId,
        Blake2_128Concat,
        [u8; 8],
        (BlockNumberFor<T>, u32, u64),
        ValueQuery,
    >;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn open_session(
            ...
            let permit: SessionPermit<T::AccountId, BlockNumberFor<T>> =
                SessionPermit::decode(&mut input).map_err(|_| Error::<T>::BadSession)?;
            // authorizer：仅允许在白名单中的赞助者代付开启
            ensure!(
                T::Authorizer::is_sponsor_allowed(permit.ns, &sponsor),
                Error::<T>::SponsorNotAllowed
            );
            ...
            if T::RequirePermitSig::get() {
                ...
                let gh = frame_system::Pallet::<T>::block_hash::<
                ...
                let sig = <T as Config>::PermitSignature::decode(&mut sig_input)
                    .map_err(|_| Error::<T>::BadSignature)?;
                ensure!(
                    sig.verify(msg.as_slice(), &derived),
                    Error::<T>::BadSignature
                );
            }

            Sessions::<T>::insert(
                (permit.owner.clone(), permit.ns),
                permit.session_id,
                permit.clone(),
            );
            SessionNonce::<T>::insert((permit.owner.clone(), permit.ns), permit.session_id, 0u64);
            SessionStats::<T>::insert(
                (permit.owner.clone(), permit.ns),
                permit.session_id,
                (0u32, 0u64),
            );
            Self::deposit_event(Event::SessionOpened {
                owner: permit.owner,
                ns: permit.ns,
        ...
        pub fn close_session(
            ...
            Self::deposit_event(Event::SessionClosed {
                owner,
                ns,
                session_id,
            });
        ...
        pub fn forward(
            ...
            // 赞助者白名单
                ...
                T::Authorizer::is_sponsor_allowed(meta.ns, &sponsor),
                Error::<T>::SponsorNotAllowed
            );

            // 会话存在与未过期
            let maybe = Sessions::<T>::get((owner.clone(), meta.ns), meta.session_id)
                .ok_or(Error::<T>::BadSession)?;
            let now = frame_system::Pallet::<T>::block_number();
            // 校验会话与 meta 的过期，同时要求 meta 至少满足最小 TTL
            if !(now <= maybe.expires_at) {
                Self::deposit_event(Event::ForwardFailed {
                    owner: owner.clone(),
            ...
            if !(now <= meta.valid_till
                && meta.valid_till.saturating_sub(now) >= T::MinMetaTxTTL::get())
            {
                Self::deposit_event(Event::ForwardFailed {
                    owner: owner.clone(),
                    sponsor: sponsor.clone(),
            ...
            // 禁止的调用过滤
                ...
                !T::ForbiddenCalls::contains(&meta.call),
                Error::<T>::ForbiddenCall
            );

            // 范围校验
            ensure!(
                T::Authorizer::is_call_allowed(meta.ns, &sponsor, &meta.call),
            ...
            let (calls, total) = SessionStats::<T>::get((owner.clone(), meta.ns), meta.session_id);
                ...
                calls < T::MaxCallsPerSession::get(),
                Error::<T>::ForbiddenCall
            );
            let next_total = total
                ...
                next_total <= T::MaxWeightPerSessionRefTime::get(),
                Error::<T>::ForbiddenCall
            );
            // 预计记账（失败也计数，避免重放薅费）
            SessionStats::<T>::insert(
            ...
            let per_blk = ForwardedPerBlock::<T>::get(now);
                ...
                per_blk < T::MaxForwardedPerBlock::get(),
                Error::<T>::ForbiddenCall
            );
            ...
            SponsorWindowStats::<T>::mutate(sponsor.clone(), meta.ns, |w| {
                ...
                let now_blk = now;
                if now_blk.saturating_sub(w.0) >= wb {
                    w.0 = now_blk;
                    w.1 = 0;
                    w.2 = 0;
                }
                ...
                w.2 =
                    w.2.saturating_add(info.call_weight.ref_time())
                        .saturating_add(info.extension_weight.ref_time());
            });
    ...
    impl<T: Config> Pallet<T> {
        /// 函数级中文注释：只读接口——返回指定区块高度下的代付调用次数统计。
        pub fn forwarded_count_at(block_number: BlockNumberFor<T>) -> u32 {
            ForwardedPerBlock::<T>::get(block_number)
        }

        /// 函数级中文注释：只读接口——返回某赞助者在特定命名空间内最近窗口的统计数据。
        pub fn sponsor_window_stats(
            sponsor: T::AccountId,
            ns: [u8; 8],
        ) -> (BlockNumberFor<T>, u32, u64) {
            SponsorWindowStats::<T>::get(sponsor, ns)
        }
    }
}
