...
pub enum TextKind { Article, Message }
...
pub mod pallet {
    use super::*;
    use frame_support::traits::{ReservableCurrency, Currency as CurrencyTrait};
    ...
    pub trait Config: frame_system::Config {
        ...
        type TextId: Parameter + Member + AtLeast32BitUnsigned + Default + Copy + MaxEncodedLen + From<u64> + Into<u64>;
        ...
        #[pallet::constant] type TextDeposit: Get<BalanceOf<Self>>;
    ...
    #[pallet::storage] pub type NextTextId<T: Config> = StorageValue<_, T::TextId, ValueQuery>;
    #[pallet::storage] pub type TextOf<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, TextRecord<T>, OptionQuery>;
    #[pallet::storage] pub type MessagesByDeceased<T: Config> = StorageMap<_, Blake2_128Concat, T::DeceasedId, BoundedVec<T::TextId, T::MaxMessagesPerDeceased>, ValueQuery>;
    #[pallet::storage] pub type ArticlesByDeceased<T: Config> = StorageMap<_, Blake2_128Concat, T::DeceasedId, BoundedVec<T::TextId, frame_support::traits::ConstU32<1024>>, ValueQuery>;

    #[pallet::storage] pub type TextDeposits<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, (T::AccountId, BalanceOf<T>), OptionQuery>;
    #[pallet::storage] pub type TextMaturity<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, BlockNumberFor<T>, OptionQuery>;
    ...
    // Eulogy（沿用 TextId 空间）
    #[pallet::storage] pub type EulogiesByDeceased<T: Config> = StorageMap<_, Blake2_128Concat, T::DeceasedId, BoundedVec<T::TextId, T::MaxEulogiesPerDeceased>, ValueQuery>;
    #[pallet::storage] pub type EulogyOf<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, (T::DeceasedId, BoundedVec<u8, T::StringLimit>, T::AccountId), OptionQuery>;
    #[pallet::storage] pub type EulogyDeposits<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, (T::AccountId, BalanceOf<T>), OptionQuery>;
    #[pallet::storage] pub type EulogyMaturity<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, BlockNumberFor<T>, OptionQuery>;
    // 投诉：生平与悼词
    ...
    #[pallet::storage] pub type EulogyComplaints<T: Config> = StorageMap<_, Blake2_128Concat, T::TextId, u32, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_article(origin: OriginFor<T>, deceased_id: T::DeceasedId, cid: Vec<u8>, title: Option<Vec<u8>>, summary: Option<Vec<u8>>) -> DispatchResult {
            ...
            let rec = TextRecord::<T> { id, deceased_id, deceased_token: token, author: who.clone(), kind: TextKind::Article, cid: cid_bv, title: title_bv, summary: summary_bv, created: now, updated: now };
            TextOf::<T>::insert(id, rec);
            ArticlesByDeceased::<T>::try_mutate(deceased_id, |list| list.try_push(id).map_err(|_| Error::<T>::TooMany))?;
            let dep = T::TextDeposit::get(); if !dep.is_zero() { T::Currency::reserve(&who, dep).map_err(|_| Error::<T>::DepositFailed)?; TextDeposits::<T>::insert(id, (who.clone(), dep)); TextMaturity::<T>::insert(id, now + T::ComplaintPeriod::get()); }
        ...
        pub fn complain_life(origin: OriginFor<T>, deceased_id: T::DeceasedId) -> DispatchResult {
            ...
            let key = (3u8, deceased_id.into()); ensure!(ComplaintOf::<T>::get(key).is_none(), Error::<T>::BadInput);
            ...
            let cnt = LifeComplaints::<T>::get(deceased_id).saturating_add(1); LifeComplaints::<T>::insert(deceased_id, cnt);
            Self::deposit_event(Event::LifeComplained(deceased_id, cnt)); Ok(())
        }
        ...
        pub fn complain_eulogy(origin: OriginFor<T>, id: T::TextId) -> DispatchResult {
            ...
            let key = (4u8, id.into()); ensure!(ComplaintOf::<T>::get(key).is_none(), Error::<T>::BadInput);
            let dep = T::ComplaintDeposit::get(); if !dep.is_zero() { T::Currency::reserve(&who, dep).map_err(|_| Error::<T>::DepositFailed)?; }
            let now = <frame_system::Pallet<T>>::block_number(); ComplaintOf::<T>::insert(key, ComplaintCase { complainant: who.clone(), deposit: dep, created: now, status: ComplaintStatus::Pending });
            let cnt = EulogyComplaints::<T>::get(id).saturating_add(1); EulogyComplaints::<T>::insert(id, cnt);
            Self::deposit_event(Event::EulogyComplained(id, cnt)); Ok(())
        }
        ...
        pub fn gov_resolve_life_complaint(origin: OriginFor<T>, deceased_id: T::DeceasedId, uphold: bool, evidence_cid: Vec<u8>) -> DispatchResult {
            ...
            if uphold {
                ...
                if let Some(prev) = LifePrev::<T>::take(deceased_id) { LifeOf::<T>::mutate(deceased_id, |life| { if let Some(l) = life { l.cid = prev; l.last_editor = None; } }); }
                if let Some((editor, d)) = LifeDeposits::<T>::take(deceased_id) { if !d.is_zero() { let win=(d*20u32.into())/100u32.into(); let fee=(d*5u32.into())/100u32.into(); let back=d-win-fee; T::Currency::repatriate_reserved(&editor, &case.complainant, win, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::repatriate_reserved(&editor, &arb, fee, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::unreserve(&editor, back); Self::deposit_event(Event::ComplaintPayoutWinner(case.complainant.clone(), win)); Self::deposit_event(Event::ComplaintPayoutArbitration(arb.clone(), fee)); Self::deposit_event(Event::ComplaintPayoutLoserRefund(editor.clone(), back)); } }
                if !case.deposit.is_zero() { T::Currency::unreserve(&case.complainant, case.deposit); }
            } else {
                let d = case.deposit; if !d.is_zero() { let win=(d*20u32.into())/100u32.into(); let fee=(d*5u32.into())/100u32.into(); let back=d-win-fee; if let Some(l) = LifeOf::<T>::get(deceased_id) { T::Currency::repatriate_reserved(&case.complainant, &l.owner, win, frame_support::traits::BalanceStatus::Free).ok(); } T::Currency::repatriate_reserved(&case.complainant, &arb, fee, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::unreserve(&case.complainant, back); Self::deposit_event(Event::ComplaintPayoutArbitration(arb.clone(), fee)); Self::deposit_event(Event::ComplaintPayoutLoserRefund(case.complainant.clone(), back)); }
            }
            case.status = ComplaintStatus::Resolved; ComplaintOf::<T>::remove(key); LifeComplaints::<T>::insert(deceased_id, 0); Self::deposit_event(Event::ComplaintResolved(3u8, deceased_id.into(), uphold)); Ok(())
        }
        ...
        pub fn gov_resolve_eulogy_complaint(origin: OriginFor<T>, id: T::TextId, uphold: bool, evidence_cid: Vec<u8>) -> DispatchResult {
            ...
            let arb = T::ArbitrationAccount::get();
            if uphold {
                if let Some((author, d)) = EulogyDeposits::<T>::take(id) { if !d.is_zero() { let win=(d*20u32.into())/100u32.into(); let fee=(d*5u32.into())/100u32.into(); let back=d-win-fee; T::Currency::repatriate_reserved(&author, &case.complainant, win, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::repatriate_reserved(&author, &arb, fee, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::unreserve(&author, back); Self::deposit_event(Event::ComplaintPayoutWinner(case.complainant.clone(), win)); Self::deposit_event(Event::ComplaintPayoutArbitration(arb.clone(), fee)); Self::deposit_event(Event::ComplaintPayoutLoserRefund(author.clone(), back)); } }
                if !case.deposit.is_zero() { T::Currency::unreserve(&case.complainant, case.deposit); }
            } else {
                let d = case.deposit; if !d.is_zero() { let win=(d*20u32.into())/100u32.into(); let fee=(d*5u32.into())/100u32.into(); let back=d-win-fee; if let Some((author,_))=EulogyDeposits::<T>::get(id) { T::Currency::repatriate_reserved(&case.complainant, &author, win, frame_support::traits::BalanceStatus::Free).ok(); } T::Currency::repatriate_reserved(&case.complainant, &arb, fee, frame_support::traits::BalanceStatus::Free).ok(); T::Currency::unreserve(&case.complainant, back); Self::deposit_event(Event::ComplaintPayoutArbitration(arb.clone(), fee)); Self::deposit_event(Event::ComplaintPayoutLoserRefund(case.complainant.clone(), back)); }
            }
            ComplaintOf::<T>::remove(key); EulogyComplaints::<T>::insert(id, 0); Self::deposit_event(Event::ComplaintResolved(4u8, id.into(), uphold)); Ok(())
        }
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn add_message(origin: OriginFor<T>, deceased_id: T::DeceasedId, cid: Vec<u8>, title: Option<Vec<u8>>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(T::DeceasedProvider::deceased_exists(deceased_id), Error::<T>::DeceasedNotFound);
            let cid_bv: BoundedVec<_, T::StringLimit> = BoundedVec::try_from(cid).map_err(|_| Error::<T>::BadInput)?;
            let title_bv = match title { Some(v)=>Some(BoundedVec::try_from(v).map_err(|_| Error::<T>::BadInput)?), None=>None };
            let id = NextTextId::<T>::get(); NextTextId::<T>::put(id.saturating_add(T::TextId::from(1u64)));
            let now = <frame_system::Pallet<T>>::block_number();
            let token = T::DeceasedTokenProvider::token_of(deceased_id).unwrap_or_default();
            let rec = TextRecord::<T> { id, deceased_id, deceased_token: token, author: who.clone(), kind: TextKind::Message, cid: cid_bv, title: title_bv, summary: None, created: now, updated: now };
            TextOf::<T>::insert(id, rec);
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn edit_text(origin: OriginFor<T>, id: T::TextId, cid: Option<Vec<u8>>, title: Option<Option<Vec<u8>>>, summary: Option<Option<Vec<u8>>>) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn claim_text_deposit(origin: OriginFor<T>, id: T::TextId) -> DispatchResult {
            ...
            let now = <frame_system::Pallet<T>>::block_number(); ensure!(now >= mature_at, Error::<T>::NotMatured);
            T::Currency::unreserve(&who, amt);
            TextDeposits::<T>::remove(id); TextMaturity::<T>::remove(id);
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn create_life(origin: OriginFor<T>, deceased_id: T::DeceasedId, cid: Vec<u8>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(T::DeceasedProvider::deceased_exists(deceased_id), Error::<T>::DeceasedNotFound);
            ensure!(T::DeceasedProvider::can_manage(&who, deceased_id), Error::<T>::NotAuthorized);
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn update_life(origin: OriginFor<T>, deceased_id: T::DeceasedId, cid: Vec<u8>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            LifeOf::<T>::try_mutate(deceased_id, |maybe| -> DispatchResult {
                let life = maybe.as_mut().ok_or(Error::<T>::BadInput)?;
                let bv: BoundedVec<_, T::StringLimit> = BoundedVec::try_from(cid).map_err(|_| Error::<T>::BadInput)?;
                let now = <frame_system::Pallet<T>>::block_number();
                if who == life.owner { life.cid = bv; life.updated = now; life.version = life.version.saturating_add(1); life.last_editor = None; } else {
                    let dep = T::TextDeposit::get(); if !dep.is_zero() { T::Currency::reserve(&who, dep).map_err(|_| Error::<T>::DepositFailed)?; LifeDeposits::<T>::insert(deceased_id, (who.clone(), dep)); LifeMaturity::<T>::insert(deceased_id, now + T::ComplaintPeriod::get()); }
        ...
        pub fn claim_life_deposit(origin: OriginFor<T>, deceased_id: T::DeceasedId) -> DispatchResult {
            ...
            let now = <frame_system::Pallet<T>>::block_number(); ensure!(now >= mature_at, Error::<T>::NotMatured);
            T::Currency::unreserve(&who, amt);
            LifeDeposits::<T>::remove(deceased_id); LifeMaturity::<T>::remove(deceased_id);
        ...
        pub fn create_eulogy(origin: OriginFor<T>, deceased_id: T::DeceasedId, cid: Vec<u8>) -> DispatchResult {
            ...
            let cid_bv: BoundedVec<_, T::StringLimit> = BoundedVec::try_from(cid).map_err(|_| Error::<T>::BadInput)?;
            let id = NextTextId::<T>::get(); let next = id.saturating_add(T::TextId::from(1u64)); NextTextId::<T>::put(next);
            let token = T::DeceasedTokenProvider::token_of(deceased_id).unwrap_or_default(); let now = <frame_system::Pallet<T>>::block_number();
            ...
            let dep = T::TextDeposit::get(); if !dep.is_zero() { T::Currency::reserve(&who, dep).map_err(|_| Error::<T>::DepositFailed)?; EulogyDeposits::<T>::insert(id, (who.clone(), dep)); EulogyMaturity::<T>::insert(id, now + T::ComplaintPeriod::get()); }
            let _ = token; // 仅为保持接口一致，目前未直接存入悼词记录
            Self::deposit_event(Event::EulogyCreated(id, deceased_id, who)); Ok(())
        }
        ...
        pub fn update_eulogy(origin: OriginFor<T>, id: T::TextId, cid: Vec<u8>) -> DispatchResult {
            ...
            Self::deposit_event(Event::EulogyUpdated(id)); Ok(())
        }
        ...
        pub fn gov_remove_eulogy(origin: OriginFor<T>, id: T::TextId, evidence_cid: Vec<u8>) -> DispatchResult {
            ...
            Self::deposit_event(Event::EulogyRemoved(id)); Ok(())
        }
        ...
        pub fn claim_eulogy_deposit(origin: OriginFor<T>, id: T::TextId) -> DispatchResult {
            ...
            let now = <frame_system::Pallet<T>>::block_number(); ensure!(now >= mature_at, Error::<T>::NotMatured);
            T::Currency::unreserve(&who, amt);
            EulogyDeposits::<T>::remove(id); EulogyMaturity::<T>::remove(id);
            Self::deposit_event(Event::EulogyDepositRefunded(id, who, amt)); Ok(())
        }
        ...
        pub fn gov_set_article_for(origin: OriginFor<T>, owner: T::AccountId, deceased_id: T::DeceasedId, cid: Vec<u8>, title: Option<Vec<u8>>, summary: Option<Vec<u8>>, evidence_cid: Vec<u8>) -> DispatchResult {
            ...
            let summary_bv = match summary { Some(v)=>Some(BoundedVec::try_from(v).map_err(|_| Error::<T>::BadInput)?), None=>None };
            let id = NextTextId::<T>::get(); NextTextId::<T>::put(id.saturating_add(T::TextId::from(1u64)));
            let now = <frame_system::Pallet<T>>::block_number(); let token = T::DeceasedTokenProvider::token_of(deceased_id).unwrap_or_default();
            let rec = TextRecord::<T> { id, deceased_id, deceased_token: token, author: owner.clone(), kind: TextKind::Article, cid: cid_bv, title: title_bv, summary: summary_bv, created: now, updated: now };
            TextOf::<T>::insert(id, rec); ArticlesByDeceased::<T>::try_mutate(deceased_id, |list| list.try_push(id).map_err(|_| Error::<T>::TooMany))?;
            let dep = T::TextDeposit::get(); if !dep.is_zero() { T::Currency::reserve(&owner, dep).map_err(|_| Error::<T>::DepositFailed)?; TextDeposits::<T>::insert(id, (owner.clone(), dep)); TextMaturity::<T>::insert(id, now + T::ComplaintPeriod::get()); }
            Self::deposit_event(Event::ArticleSet(id, deceased_id, owner)); Ok(())
        }
    }
}
