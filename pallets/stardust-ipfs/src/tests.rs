...
/// 函数级中文注释：测试14 - 四层回退扣费（IpfsPool不足，回退到SubjectFunding）
#[test]
...
/// 函数级中文注释：测试20 - pin_cid_for_grave复用deceased逻辑
#[test]
fn pin_for_grave_reuses_deceased_logic() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // 通过trait调用pin_cid_for_grave
        assert_ok!(crate::Pallet::<Test>::pin_cid_for_grave(
            caller,
            grave_id,
            cid.clone(),
            Some(PinTier::Standard),
        ...
        // 验证CID已注册（使用pseudo_deceased_id = u64::MAX - grave_id）
        use sp_runtime::traits::Hash;
        let cid_hash = BlakeTwo256::hash(&cid);
        assert!(crate::PinMeta::<Test>::contains_key(cid_hash));
    });
