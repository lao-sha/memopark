...
pub mod v2 {
    ...
    impl<T: Config> LazyMigrationV1ToV2<T> {
        pub(crate) fn required_weight(
            ...
            match step {
                ...
                | MigrationState::CleanupAuthorities(_) => {
                    T::WeightInfo::migration_v2_cleanup_authority_step()
                }
                MigrationState::FinishedCleanupAuthorities
                | MigrationState::CleanupUsernames(_) => {
                    T::WeightInfo::migration_v2_cleanup_username_step()
                }
        ...
        pub(crate) fn authority_step(maybe_last_key: Option<&T::AccountId>) -> StepResultOf<T> {
            ...
            if let Some((authority_account, properties)) = iter.next() {
                ...
                let new_properties = AuthorityProperties {
                    account_id: authority_account.clone(),
                    allocation,
                };
        ...
        pub(crate) fn identity_step(maybe_last_key: Option<HashedKey>) -> StepResultOf<T> {
            ...
            >(
                ...
                |account, (identity, maybe_username)| {
    ...
    impl<T: Config> LazyMigrationV1ToV2<T> {
        pub(crate) fn setup_benchmark_env_for_migration() -> BenchmarkingSetupOf<T> {
            ...
            let prop: AuthorityProperties<Suffix<T>> = AuthorityProperties {
                account_id: suffix.clone(),
                allocation: 10,
            };
        ...
        pub(crate) fn setup_benchmark_env_for_cleanup() -> BenchmarkingSetupOf<T> {
            ...
            types_v1::UsernameAuthorities::<T>::insert(&authority, &prop);
            let prop: AuthorityProperties<T::AccountId> = AuthorityProperties {
        ...
        pub(crate) fn check_authority_cleanup_validity(suffix: Suffix<T>, authority: T::AccountId) {
            ...
            assert_eq!(
                AuthorityOf::<T>::get(&suffix).unwrap().account_id,
                authority
            );
        }
        ...
        pub(crate) fn check_username_cleanup_validity(
            ...
            assert_eq!(
                UsernameInfoOf::<T>::get(&username).unwrap().owner,
                account_id
            );
        }
    }
    ...
    mod tests {
        ...
        fn migrate_to_v2() {
            new_test_ext().execute_with(|| {
                ...
                let prop = AuthorityProperties {
                    account_id: suffix_1.clone(),
                    allocation: 10,
                };
                ...
                let expected_prop = AuthorityProperties {
                ...
                for (username, owner, since) in pending.iter() {
                    ...
                    assert_eq!(
                        PendingUsernames::<Test>::get(username),
