...
fn editing_subaccounts_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SuperOf::<Test>::get(one.clone()),
            Some((ten.clone(), data(1)))
        );
        assert_eq!(
            Balances::free_balance(ten.clone()),
            1000 - id_deposit - sub_deposit
        );
        ...
        assert_eq!(
            SuperOf::<Test>::get(two.clone()),
            Some((ten.clone(), data(2)))
        );
        assert_eq!(
            Balances::free_balance(ten.clone()),
            1000 - id_deposit - 2 * sub_deposit
        );
...
fn resolving_subaccount_ownership_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(twenty.clone()),
            1000 - twenty_deposit
        );
        ...
        assert_eq!(
            Balances::free_balance(ten.clone()),
            1000 - ten_deposit - sub_deposit
        );
        assert_eq!(
            Balances::reserved_balance(ten.clone()),
            ten_deposit + sub_deposit
        );
...
fn killing_slashing_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Identity::kill_identity(RuntimeOrigin::signed(one), ten.clone()),
            BadOrigin
        );
...
fn cancelling_requested_judgement_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Identity::cancel_request(
            RuntimeOrigin::signed(ten.clone()),
            0
        ));
        assert_eq!(
...
fn setting_account_id_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Identity::set_account_id(
            RuntimeOrigin::signed(three.clone()),
            0,
            four.clone()
        ));
...
fn poke_deposit_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SubsOf::<Test>::get(ten),
            (subs_deposit, vec![twenty].try_into().unwrap())
        );
    });
}
...
fn set_username_with_signature_without_existing_identity_should_work() {
    new_test_ext().execute_with(|| {
        ...
        // them.
        assert_eq!(
            UsernameOf::<Test>::get(&second_who),
            Some(second_username.clone())
        );
        // Lookup from username to account works.
        let expected_user_info = UsernameInformation {
            owner: second_who,
...
fn set_username_through_deposit_with_existing_identity_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::reserved_balance(&who_account),
            expected_identity_deposit
        );
        assert_ok!(Identity::set_username_for(
            RuntimeOrigin::signed(authority.clone()),
        ...
        assert_eq!(
            Balances::free_balance(&who_account),
            1000 - expected_identity_deposit
        );
        assert_eq!(
            Balances::reserved_balance(&who_account),
            expected_identity_deposit
...
fn set_username_with_acceptance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        // Check Identity storage
        assert_eq!(
            UsernameOf::<Test>::get(&second_caller),
            Some(second_username.clone())
        );
        // Check reverse lookup
        let expected_user_info = UsernameInformation {
...
fn must_own_primary() {
    new_test_ext().execute_with(|| {
        ...
        let expected_pi_info = UsernameInformation {
            owner: pi_account.clone(),
            provider: Provider::Allocation,
        };
        assert_eq!(
        ...
        let expected_e_info = UsernameInformation {
            owner: e_account.clone(),
            provider: Provider::Allocation,
        };
        assert_eq!(
...
fn kill_username_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(second_authority.clone()),
            initial_authority_balance
        );
        ...
        assert_ok!(Identity::kill_username(
            RuntimeOrigin::root(),
            username_two.clone().into()
        ));
        ...
        assert_eq!(
            Balances::reserved_balance(authority.clone()),
            username_deposit
        );
...
fn unbind_and_remove_username_should_work() {
    new_test_ext().execute_with(|| {
        ...
        AuthorityOf::<Test>::insert(
            dummy_suffix.clone(),
            AuthorityProperties {
                account_id: dummy_authority.clone(),
                allocation: 10,
            },
        );
        ...
        let expected_grace_period_expiry: BlockNumberFor<Test> = now + grace_period;
        assert_eq!(
            UnbindingUsernames::<Test>::get(&username),
            Some(expected_grace_period_expiry)
        );
        // Advance the block number to simulate the grace period passing.
        System::set_block_number(expected_grace_period_expiry);
