...
pub mod pallet {
    use super::*;
    use alloc::vec::Vec;
    use frame_support::{pallet_prelude::*, traits::ReservableCurrency, BoundedVec};
    ...
    #[pallet::storage]
    pub type SacrificeOf<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, SacrificeItem<T>, OptionQuery>;
    ...
    /// 函数级中文注释：场景存储与索引。
    #[pallet::storage]
    pub type NextSceneId<T: Config> = StorageValue<_, u32, ValueQuery>;
    #[pallet::storage]
    pub type SceneOf<T: Config> = StorageMap<_, Blake2_128Concat, u32, Scene<T>, OptionQuery>;
    /// 按域索引（可选）：domain -> [scene_id]
    #[pallet::storage]
    pub type ScenesByDomain<T: Config> =
        StorageMap<_, Blake2_128Concat, u8, BoundedVec<u32, ConstU32<1000>>, ValueQuery>;

    /// 函数级中文注释：押金/成熟/投诉占位（与 Data/Life/Eulogy 风格保持一致）
    #[pallet::storage]
    pub type SacrificeDeposits<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, (T::AccountId, BalanceOf<T>), OptionQuery>;
    #[pallet::storage]
    pub type SacrificeMaturity<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BlockNumberFor<T>, OptionQuery>;
    #[pallet::storage]
    pub type SacrificeComplaints<T: Config> = StorageMap<_, Blake2_128Concat, u64, u32, ValueQuery>;
    ...
    #[pallet::storage]
    pub type EffectOf<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, (bool, u8, u8, i32, u32, bool), OptionQuery>;
    ...
    pub type CategoryOf<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        u32,
        (u32, BoundedVec<u8, T::StringLimit>, Option<u32>, u8),
        OptionQuery,
    >;
    #[pallet::storage]
    pub type NextCategoryId<T: Config> = StorageValue<_, u32, ValueQuery>;
    /// 父子关系索引 parent_id => [child_id]
    #[pallet::storage]
    pub type ChildrenByCategory<T: Config> =
        StorageMap<_, Blake2_128Concat, u32, BoundedVec<u32, ConstU32<100>>, ValueQuery>;
    /// 反向索引：一级类目
    #[pallet::storage]
    pub type SacrificesByPrimary<T: Config> =
        StorageMap<_, Blake2_128Concat, u32, BoundedVec<u64, ConstU32<10000>>, ValueQuery>;
    /// 反向索引：二级类目
    #[pallet::storage]
    pub type SacrificesBySecondary<T: Config> =
        StorageMap<_, Blake2_128Concat, u32, BoundedVec<u64, ConstU32<10000>>, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_sacrifice(
            ...
            if let Some(sid) = scene_id {
                let sc = SceneOf::<T>::get(sid).ok_or(Error::<T>::SceneNotFound)?;
                ensure!(sc.active, Error::<T>::SceneInactive);
            }
        ...
        pub fn request_list_sacrifice(
            ...
            let desc_bv: BoundedVec<_, T::DescriptionLimit> =
                BoundedVec::try_from(description).map_err(|_| Error::<T>::BadInput)?;
            let exclusive_bv: BoundedVec<(u8, u64), T::MaxExclusivePerItem> =
                BoundedVec::try_from(exclusive_subjects).map_err(|_| Error::<T>::BadInput)?;
            // 场景校验：若提供 scene_id，要求存在且 active
            if let Some(sid) = scene_id {
                let sc = SceneOf::<T>::get(sid).ok_or(Error::<T>::SceneNotFound)?;
                ensure!(sc.active, Error::<T>::SceneInactive);
        ...
        pub fn committee_approve(origin: OriginFor<T>, id: u64) -> DispatchResult {
            ...
            if let Some((owner, amt)) = SacrificeDeposits::<T>::take(id) {
                if !amt.is_zero() {
                    T::Currency::unreserve(&owner, amt);
                }
            }
        ...
        pub fn committee_reject(origin: OriginFor<T>, id: u64) -> DispatchResult {
            ...
            if let Some((owner, amt)) = SacrificeDeposits::<T>::take(id) {
                if !amt.is_zero() {
                    ...
                    if !back.is_zero() {
                        T::Currency::unreserve(&owner, back);
                    }
                }
            }
            SacrificeOf::<T>::try_mutate(id, |maybe| -> DispatchResult {
        ...
        pub fn create_category(
            ...
            if let Some(pid) = parent {
                let (_id, _n, pparent, plevel) =
                    CategoryOf::<T>::get(pid).ok_or(Error::<T>::BadInput)?;
                ensure!(pparent.is_none() && plevel == 1, Error::<T>::BadInput);
            }
            let id = NextCategoryId::<T>::mutate(|n| {
                let x = *n;
                *n = x.saturating_add(1);
                x
            });
            ...
            if let Some(pid) = parent {
                ChildrenByCategory::<T>::try_mutate(pid, |v| {
                    v.try_push(id).map_err(|_| Error::<T>::BadInput)
                })?;
            }
            Ok(())
        }
        ...
        pub fn update_category(
            ...
            CategoryOf::<T>::try_mutate(id, |maybe| -> DispatchResult {
                ...
                if let Some(n) = name {
                    *cname = BoundedVec::try_from(n).map_err(|_| Error::<T>::BadInput)?;
                }
                if let Some(pp) = parent {
                    match (clevel, pp) {
                        ...
                        (2, Some(newp)) => {
                            let (_pid, _pn, pparent, plevel) =
                                CategoryOf::<T>::get(newp).ok_or(Error::<T>::BadInput)?;
                            ensure!(pparent.is_none() && plevel == 1, Error::<T>::BadInput);
                            if let Some(oldp) = *cparent {
                                ChildrenByCategory::<T>::mutate(oldp, |v| {
                                    if let Some(pos) = v.iter().position(|x| x == cid) {
                                        v.swap_remove(pos);
                                    }
                                });
                            }
                            ChildrenByCategory::<T>::try_mutate(newp, |v| {
                                v.try_push(*cid).map_err(|_| Error::<T>::BadInput)
                            })?;
                            *cparent = Some(newp);
                        }
        ...
        pub fn assign_category(
            ...
            SacrificeOf::<T>::try_mutate(id, |maybe| -> DispatchResult {
                ...
                if let Some(p) = primary {
                    let (_pid, _n, pparent, plevel) =
                        CategoryOf::<T>::get(p).ok_or(Error::<T>::BadInput)?;
                    ensure!(pparent.is_none() && plevel == 1, Error::<T>::BadInput);
                }
                if let Some(sec) = secondary {
                    let (_sid, _sn, sparent, slevel) =
                        CategoryOf::<T>::get(sec).ok_or(Error::<T>::BadInput)?;
                    if let Some(p) = primary {
                        ensure!(sparent == Some(p) && slevel == 2, Error::<T>::BadInput);
                    } else {
                        return Err(Error::<T>::BadInput.into());
                    }
                }
                // 旧索引移除
                if let Some(op) = s.primary_category_id {
                    SacrificesByPrimary::<T>::mutate(op, |v| {
                        if let Some(pos) = v.iter().position(|x| x == &id) {
                            v.swap_remove(pos);
                        }
                    });
                }
                if let Some(os) = s.secondary_category_id {
                    SacrificesBySecondary::<T>::mutate(os, |v| {
                        if let Some(pos) = v.iter().position(|x| x == &id) {
                            v.swap_remove(pos);
                        }
                    });
                ...
                // 新索引写入
                if let Some(p) = primary {
                    SacrificesByPrimary::<T>::try_mutate(p, |v| {
                        v.try_push(id).map_err(|_| Error::<T>::BadInput)
                    })?;
                }
                if let Some(sec) = secondary {
                    SacrificesBySecondary::<T>::try_mutate(sec, |v| {
                        v.try_push(id).map_err(|_| Error::<T>::BadInput)
                    })?;
                }
                s.primary_category_id = primary;
                s.secondary_category_id = secondary;
                s.updated = <frame_system::Pallet<T>>::block_number();
                Ok(())
            })?;
            Ok(())
        ...
        pub fn set_effect(
            ...
            match effect {
                Some(v) => {
                    EffectOf::<T>::insert(id, v);
                }
                None => {
                    EffectOf::<T>::remove(id);
                }
            }
            Ok(())
        }
        ...
        pub fn set_status(origin: OriginFor<T>, id: u64, status: u8) -> DispatchResult {
            ...
            let code: u8 = match st {
                SacrificeStatus::Enabled => 0,
                SacrificeStatus::Disabled => 1,
                SacrificeStatus::Hidden => 2,
            };
        ...
        pub fn claim_deposit(origin: OriginFor<T>, id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let (owner, amt) =
                SacrificeDeposits::<T>::get(id).ok_or(Error::<T>::NoDepositToClaim)?;
            ensure!(who == owner, DispatchError::BadOrigin);
            ensure!(
                SacrificeComplaints::<T>::get(id) == 0,
                Error::<T>::NotMatured
            );
        ...
        pub fn create_scene(
            ...
            let name_bv: BoundedVec<_, T::StringLimit> =
                BoundedVec::try_from(name).map_err(|_| Error::<T>::BadInput)?;
            let desc_bv: BoundedVec<_, T::DescriptionLimit> =
                BoundedVec::try_from(desc.unwrap_or_default()).map_err(|_| Error::<T>::BadInput)?;
            let id = NextSceneId::<T>::mutate(|n| {
                let x = *n;
                *n = x.saturating_add(1);
                x
            });
            ...
            if let Some(d) = sc.domain {
                ScenesByDomain::<T>::try_mutate(d, |v| {
                    v.try_push(id).map_err(|_| Error::<T>::BadInput)
                })?;
            }
        ...
        pub fn update_scene(
            ...
            SceneOf::<T>::try_mutate(id, |maybe| -> DispatchResult {
                ...
                if let Some(n) = name {
                    sc.name = BoundedVec::try_from(n).map_err(|_| Error::<T>::BadInput)?;
                }
                if let Some(dv) = desc {
                    sc.desc = BoundedVec::try_from(dv).map_err(|_| Error::<T>::BadInput)?;
                }
                if let Some(dom) = domain {
                    // 同步索引：从旧域移除，加入新域
                    if let Some(old) = sc.domain {
                        ScenesByDomain::<T>::mutate(old, |v| {
                    ...
                    sc.domain = dom;
                    if let Some(nd) = sc.domain {
                        ScenesByDomain::<T>::try_mutate(nd, |v| {
                            v.try_push(id).map_err(|_| Error::<T>::BadInput)
                        })?;
                    }
                }
    ...
    impl<T: Config>
        pallet_memo_offerings::pallet::SacrificeCatalog<T::AccountId, u64, u128, BlockNumberFor<T>>
        for Pallet<T>
    {
        /// 函数级中文注释：读取祭祀品定价与可用性。
        ...
        fn effect_of(id: u64) -> Option<pallet_memo_offerings::pallet::EffectSpec> {
            EffectOf::<T>::get(id).map(
                |(
                    ...
                    pallet_memo_offerings::pallet::EffectSpec {
                        consumable,
                        target_domain,
                        effect_kind,
                        effect_value,
