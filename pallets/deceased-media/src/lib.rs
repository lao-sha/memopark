...
pub mod pallet {
    ...
    use frame_support::traits::{
        Currency as CurrencyTrait, ExistenceRequirement, ReservableCurrency,
    };
    ...
    pub trait Config: frame_system::Config {
        ...
        type AlbumId: Parameter
            ...
            + From<u64>
            + Into<u64>;
        type VideoCollectionId: Parameter
            + Member
            + AtLeast32BitUnsigned
            + Default
            + Copy
    ...
    #[pallet::storage]
    pub type AlbumOf<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AlbumId, Album<T>, OptionQuery>;
    #[pallet::storage]
    pub type VideoCollectionOf<T: Config> =
        StorageMap<_, Blake2_128Concat, T::VideoCollectionId, VideoCollection<T>, OptionQuery>;
    #[pallet::storage]
    pub type MediaOf<T: Config> =
        StorageMap<_, Blake2_128Concat, T::MediaId, Media<T>, OptionQuery>;
    #[pallet::storage]
    ...
    >;
    #[pallet::storage]
    pub type MediaByVideoCollection<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        T::VideoCollectionId,
        BoundedVec<T::MediaId, T::MaxPhotoPerAlbum>,
    ...
    #[pallet::storage]
    pub type AlbumDeposits<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AlbumId, (T::AccountId, BalanceOf<T>), OptionQuery>;
    #[pallet::storage]
    pub type VideoCollectionDeposits<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        T::VideoCollectionId,
    ...
    >;
    #[pallet::storage]
    pub type MediaDeposits<T: Config> =
        StorageMap<_, Blake2_128Concat, T::MediaId, (T::AccountId, BalanceOf<T>), OptionQuery>;
    #[pallet::storage]
    pub type AlbumMaturity<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AlbumId, BlockNumberFor<T>, OptionQuery>;
    #[pallet::storage]
    pub type VideoCollectionMaturity<T: Config> =
        StorageMap<_, Blake2_128Concat, T::VideoCollectionId, BlockNumberFor<T>, OptionQuery>;
    #[pallet::storage]
    pub type MediaMaturity<T: Config> =
        StorageMap<_, Blake2_128Concat, T::MediaId, BlockNumberFor<T>, OptionQuery>;
    #[pallet::storage]
    pub type AlbumFrozen<T: Config> = StorageMap<_, Blake2_128Concat, T::AlbumId, bool, ValueQuery>;
    /// 函数级中文注释：视频集冻结标记（治理设定）。冻结时禁止写操作。
    #[pallet::storage]
    pub type VideoCollectionFrozen<T: Config> =
        StorageMap<_, Blake2_128Concat, T::VideoCollectionId, bool, ValueQuery>;
    #[pallet::storage]
    pub type MediaHidden<T: Config> = StorageMap<_, Blake2_128Concat, T::MediaId, bool, ValueQuery>;
    /// 函数级中文注释：逝者主图（仅 Photo 类型媒体）。使用媒体域统一管理，避免侵入主体档案。
    #[pallet::storage]
    pub type PrimaryImageOfDeceased<T: Config> =
        StorageMap<_, Blake2_128Concat, T::DeceasedId, T::MediaId, OptionQuery>;
    ...
    /// 函数级中文注释：相册与媒体的投诉计数。
    #[pallet::storage]
    pub type AlbumComplaints<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AlbumId, u32, ValueQuery>;
    #[pallet::storage]
    pub type MediaComplaints<T: Config> =
        StorageMap<_, Blake2_128Concat, T::MediaId, u32, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_album(
            ...
            let title_bv: BoundedVec<_, T::StringLimit> =
                BoundedVec::try_from(title).map_err(|_| Error::<T>::BadInput)?;
            let desc_bv: BoundedVec<_, T::StringLimit> =
                BoundedVec::try_from(desc).map_err(|_| Error::<T>::BadInput)?;
            let mut tags_bv: BoundedVec<BoundedVec<u8, T::StringLimit>, T::MaxTags> =
                Default::default();
            for t in tags.into_iter() {
                let tb: BoundedVec<_, T::StringLimit> =
                    BoundedVec::try_from(t).map_err(|_| Error::<T>::BadInput)?;
            ...
            AlbumsByDeceased::<T>::try_mutate(deceased_id, |list| {
            ...
            if !dep.is_zero() {
                ...
                AlbumMaturity::<T>::insert(id, now + T::ComplaintPeriod::get());
            }
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn clear_primary_image_for(
            origin: OriginFor<T>,
            deceased_id: T::DeceasedId,
        ) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn gov_set_primary_image_for(
            origin: OriginFor<T>,
            deceased_id: T::DeceasedId,
            media_id: Option<T::MediaId>,
            evidence_cid: Vec<u8>,
        ) -> DispatchResult {
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn set_album_primary_photo(
            origin: OriginFor<T>,
            album_id: T::AlbumId,
            media: Option<T::MediaId>,
        ) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn gov_set_album_primary_photo(
            origin: OriginFor<T>,
            album_id: T::AlbumId,
            media: Option<T::MediaId>,
            evidence_cid: Vec<u8>,
        ...
        pub fn complain_album(origin: OriginFor<T>, album_id: T::AlbumId) -> DispatchResult {
            let who = ensure_signed(origin)?;
                ...
                AlbumOf::<T>::contains_key(album_id),
        ...
        pub fn complain_media(origin: OriginFor<T>, media_id: T::MediaId) -> DispatchResult {
            let who = ensure_signed(origin)?;
                ...
                MediaOf::<T>::contains_key(media_id) || MediaDeposits::<T>::contains_key(media_id),
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn gov_resolve_album_complaint(
            origin: OriginFor<T>,
            album_id: T::AlbumId,
            uphold: bool,
            evidence_cid: Vec<u8>,
            ...
            let (uploader, upload_dep) = AlbumDeposits::<T>::get(album_id)
                .unwrap_or_else(|| (T::FeeCollector::get(), Zero::zero()));
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn gov_resolve_media_complaint(
            origin: OriginFor<T>,
            media_id: T::MediaId,
            uphold: bool,
            evidence_cid: Vec<u8>,
            ...
            let key = (2u8, media_id.into());
            let mut case = ComplaintOf::<T>::get(key).ok_or(Error::<T>::BadInput)?;
            let (uploader, upload_dep) = MediaDeposits::<T>::get(media_id)
                .unwrap_or_else(|| (T::FeeCollector::get(), Zero::zero()));
            let arb = T::ArbitrationAccount::get();
            if uphold {
                let d = upload_dep;
        ...
        pub fn create_video_collection(
            ...
            VideoCollectionsByDeceased::<T>::try_mutate(deceased_id, |list| {
                list.try_push(id).map_err(|_| Error::<T>::TooMany)
            })?;
            ...
            match kind_enum {
                MediaKind::Photo => {
                    ...
                    if let (Some(w), Some(h)) = (width, height) {
                        ensure!(w > 0 && h > 0, Error::<T>::BadInput);
                    }
                    ...
                    let now = <frame_system::Pallet<T>>::block_number();
                    let token =
                        T::DeceasedTokenProvider::token_of(album.deceased_id).unwrap_or_default();
                    let media = Media::<T> {
                        id,
                        album_id: Some(aid),
                ...
                MediaKind::Video | MediaKind::Audio => {
                    ...
                    let vsid: T::VideoCollectionId =
                        container_id.saturated_into::<u64>().saturated_into();
                    let vs = VideoCollectionOf::<T>::get(vsid)
                        .ok_or(Error::<T>::VideoCollectionNotFound)?;
                    ensure!(vs.owner == who, Error::<T>::NotAuthorized);
                    ensure!(!VideoCollectionFrozen::<T>::get(vsid), Error::<T>::Frozen);
                    if let Some(d) = duration_secs {
                        ensure!(d > 0u32, Error::<T>::BadInput);
                    }
                    let id = NextMediaId::<T>::get();
                    let next = id.saturating_add(T::MediaId::from(1u64));
                    NextMediaId::<T>::put(next);
                    ...
                    let now = <frame_system::Pallet<T>>::block_number();
                    let token =
                        T::DeceasedTokenProvider::token_of(vs.deceased_id).unwrap_or_default();
                    let media = Media::<T> {
                        id,
                        album_id: None,
                        video_collection_id: Some(vsid),
                        deceased_id: vs.deceased_id,
                        deceased_token: token,
                        owner: who.clone(),
                        kind: kind_enum,
                        uri: uri_bv,
        ...
        pub fn update_media(
            ...
            MediaOf::<T>::try_mutate(media_id, |maybe| -> DispatchResult {
                ...
                if let Some(u) = uri {
                    m.uri = BoundedVec::try_from(u).map_err(|_| Error::<T>::BadInput)?;
                }
                if let Some(t) = thumbnail_uri {
                    m.thumbnail_uri = match t {
                        Some(v) => Some(BoundedVec::try_from(v).map_err(|_| Error::<T>::BadInput)?),
                        None => None,
                    };
                }
                ...
                if let Some(dur) = duration_secs {
                    if matches!(m.kind, MediaKind::Video | MediaKind::Audio) {
                        if let Some(x) = dur {
                            ensure!(x > 0u32, Error::<T>::BadInput);
                        }
                    }
                    m.duration_secs = dur;
                }
        ...
        pub fn remove_media(origin: OriginFor<T>, media_id: T::MediaId) -> DispatchResult {
            ...
            if let Some(aid) = m.album_id {
                ...
                MediaByAlbum::<T>::mutate(aid, |list| {
                    if let Some(pos) = list.iter().position(|x| *x == media_id) {
            ...
            if let Some(vsid) = m.video_collection_id {
                ...
                MediaByVideoCollection::<T>::mutate(vsid, |list| {
                    if let Some(pos) = list.iter().position(|x| *x == media_id) {
                        list.swap_remove(pos);
                    }
                });
            ...
            if matches!(m.kind, MediaKind::Photo) {
                ...
                // 若为相册封面则清空
                if let Some(aid) = m.album_id {
                    if let Some(mut a) = AlbumOf::<T>::get(aid) {
                        if a.primary_photo_id == Some(media_id) {
                            a.primary_photo_id = None;
                            a.updated = <frame_system::Pallet<T>>::block_number();
                            AlbumOf::<T>::insert(aid, a);
                            Self::deposit_event(Event::AlbumPrimaryChanged(aid, None));
                        }
                    }
                }
            }
            ...
            if matches!(m.kind, MediaKind::Video | MediaKind::Audio) {
                if let Some(vsid) = m.video_collection_id {
                    if let Some(mut v) = VideoCollectionOf::<T>::get(vsid) {
                        if v.primary_video_id == Some(media_id) {
                            v.primary_video_id = None;
                            v.updated = <frame_system::Pallet<T>>::block_number();
                            v.version = v.version.saturating_add(1);
                            VideoCollectionOf::<T>::insert(vsid, v);
                            Self::deposit_event(Event::VideoCollectionPrimaryChanged(vsid, None));
                        }
                    }
                }
            }
        ...
        pub fn claim_album_deposit(origin: OriginFor<T>, album_id: T::AlbumId) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let (owner, amt) =
                AlbumDeposits::<T>::get(album_id).ok_or(Error::<T>::NoDepositToClaim)?;
        ...
        pub fn claim_media_deposit(origin: OriginFor<T>, media_id: T::MediaId) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let (owner, amt) =
                MediaDeposits::<T>::get(media_id).ok_or(Error::<T>::NoDepositToClaim)?;
            ensure!(who == owner, Error::<T>::NotAuthorized);
                ...
                if let Some(m) = MediaOf::<T>::get(media_id) {
                    ...
                    if matches!(m.kind, MediaKind::Photo) {
                        if let Some(current) = PrimaryImageOfDeceased::<T>::get(m.deceased_id) {
                            if current == media_id {
                                PrimaryImageOfDeceased::<T>::remove(m.deceased_id);
                                Self::deposit_event(Event::PrimaryImageChanged(
                                    m.deceased_id,
                                    None,
                                ));
                            }
                        ...
                        if let Some(vsid) = m.video_collection_id {
                            if let Some(mut v) = VideoCollectionOf::<T>::get(vsid) {
                                if v.primary_video_id == Some(media_id) {
                                    ...
                                    v.version = v.version.saturating_add(1);
                                    VideoCollectionOf::<T>::insert(vsid, v);
                                    Self::deposit_event(Event::VideoCollectionPrimaryChanged(
                                        vsid, None,
                                    ));
                                }
                            }
        ...
        pub fn gov_remove_media(
            ...
            if let Some(aid) = existed.album_id {
                MediaByAlbum::<T>::mutate(aid, |list| {
                    if let Some(pos) = list.iter().position(|x| *x == media_id) {
                        list.swap_remove(pos);
                    }
            ...
            if let Some(vsid) = existed.video_collection_id {
                MediaByVideoCollection::<T>::mutate(vsid, |list| {
                    if let Some(pos) = list.iter().position(|x| *x == media_id) {
                        list.swap_remove(pos);
                    }
        ...
        #[allow(deprecated)]
        #[pallet::weight(10_000)]
        pub fn gov_freeze_video_collection(
            origin: OriginFor<T>,
            video_id: T::VideoCollectionId,
            frozen: bool,
            evidence_cid: Vec<u8>,
        ) -> DispatchResult {
            Self::ensure_gov(origin)?;
            let _ = Self::note_evidence(3u8, video_id.into(), evidence_cid)?;
                ...
                VideoCollectionOf::<T>::contains_key(video_id),
                Error::<T>::VideoCollectionNotFound
            );
        ...
        pub fn gov_create_album_for(
            ...
            let id = NextAlbumId::<T>::get();
            NextAlbumId::<T>::put(id.saturating_add(T::AlbumId::from(1u64)));
            let vis = match visibility {
                0 => Visibility::Public,
                1 => Visibility::Unlisted,
                2 => Visibility::Private,
            ...
            if !dep.is_zero() {
                ...
                AlbumDeposits::<T>::insert(id, (owner.clone(), dep));
                AlbumMaturity::<T>::insert(id, now + T::ComplaintPeriod::get());
            }
            Self::deposit_event(Event::AlbumCreated(id, deceased_id, owner));
            Ok(())
        }
        ...
        pub fn gov_add_media_for(
            ...
            match kind_enum {
                MediaKind::Photo => {
                    ...
                    let id = NextMediaId::<T>::get();
                    NextMediaId::<T>::put(id.saturating_add(T::MediaId::from(1u64)));
                    let mut list = MediaByAlbum::<T>::get(aid);
                    let ord = order_index.unwrap_or(list.len() as u32);
                    let now = <frame_system::Pallet<T>>::block_number();
                    let token =
                    ...
                    if !dep.is_zero() {
                        T::Currency::reserve(&owner, dep).map_err(|_| Error::<T>::DepositFailed)?;
                        MediaDeposits::<T>::insert(id, (owner.clone(), dep));
                        MediaMaturity::<T>::insert(id, now + T::ComplaintPeriod::get());
                    }
                    Self::deposit_event(Event::MediaAdded(id));
                    Ok(())
