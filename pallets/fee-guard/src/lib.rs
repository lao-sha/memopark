...
use frame_support::{
    pallet_prelude::*,
    traits::{
        Currency as CurrencyTrait, LockIdentifier, LockableCurrency, OnKilledAccount,
        WithdrawReasons,
    },
};
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type FeeOnlyAccounts<T: Config> =
    ...
    pub enum Event<T: Config> {
        ...
        MarkedFeeOnly(
            ...
            BalanceOf<T>,
            u8,
            Option<BoundedVec<u8, ConstU32<128>>>,
        ),
        /// 已解除仅手续费标记（who）。
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn mark_fee_only(
            ...
            let deny_all_but_fee =
                WithdrawReasons::all().difference(WithdrawReasons::TRANSACTION_PAYMENT);
            <T as Config>::Currency::set_lock(FEE_GUARD_ID, &who, max, deny_all_but_fee);
            FeeOnlyAccounts::<T>::insert(&who, ());
            let ev = evidence_cid
                .map(|v| BoundedVec::<u8, ConstU32<128>>::try_from(v).ok())
                .flatten();
...
pub trait AllowMarkingPolicy<AccountId> {
