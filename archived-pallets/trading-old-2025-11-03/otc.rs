...
use crate::{Config, BalanceOf, MomentOf, TronAddress};
...
pub struct Order<T: Config + frame_system::Config> {
    /// 做市商ID
    pub maker_id: u64,
    /// 做市商账户
...
pub fn do_create_order<T: Config + frame_system::Config>(
    buyer: &T::AccountId,
    maker_id: u64,
    ...
    use crate::{NextOrderId, Orders, BuyerOrders, MakerOrders, MakerApplications, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_mark_paid<T: Config + frame_system::Config>(
    buyer: &T::AccountId,
    order_id: u64,
    tron_tx_hash: Option<Vec<u8>>,
) -> DispatchResult {
    use crate::{Orders, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_release_dust<T: Config + frame_system::Config>(
    maker: &T::AccountId,
    order_id: u64,
) -> DispatchResult {
    use crate::{Orders, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_cancel_order<T: Config + frame_system::Config>(
    who: &T::AccountId,
    order_id: u64,
) -> DispatchResult {
    use crate::{Orders, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_dispute_order<T: Config + frame_system::Config>(
    who: &T::AccountId,
    order_id: u64,
) -> DispatchResult {
    use crate::{Orders, Pallet, Event, Error};
...
pub trait ArbitrationHook<T: crate::Config + frame_system::Config> {
...
impl<T: crate::Config + frame_system::Config> ArbitrationHook<T> for Pallet<T> {
    fn can_dispute(who: &T::AccountId, id: u64) -> bool {
...
pub fn calculate_first_purchase_dust_amount<T: Config + frame_system::Config>() -> Result<BalanceOf<T>, DispatchError> {
    use crate::{Error, PricingProvider};
...
/// - maker_id: 做市商ID
/// - order_id: 订单ID
pub fn release_first_purchase_quota<T: Config + frame_system::Config>(
    maker_id: u64,
    order_id: u64,
) -> DispatchResult {
    use crate::{MakerFirstPurchaseCount, MakerFirstPurchaseOrders};
...
/// 7. 更新首购配额
/// 8. 标记买家已首购
pub fn create_first_purchase<T: Config + frame_system::Config>(
    buyer: &T::AccountId,
    maker_id: u64,
    payment_commit: H256,
    contact_commit: H256,
