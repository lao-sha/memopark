...
use crate::{Config, BalanceOf, TronAddress};
...
pub struct SwapRequest<T: Config + frame_system::Config> {
    /// 兑换ID
    pub id: u64,
    /// 用户地址
...
#[derive(Encode, Decode, TypeInfo, MaxEncodedLen, Clone, PartialEq, Eq, RuntimeDebug)]
#[scale_info(skip_type_params(T))]
pub struct MakerSwapRecord<T: Config + frame_system::Config> {
    /// 兑换ID
    pub swap_id: u64,
    /// 做市商ID
    pub maker_id: u64,
...
pub fn do_swap<T: Config + frame_system::Config>(
    user: &T::AccountId,
    dust_amount: BalanceOf<T>,
    tron_address: Vec<u8>,
) -> Result<u64, DispatchError> {
    use crate::{NextSwapId, SwapRequests, BridgeAccount, MinSwapAmount, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_complete_swap<T: Config + frame_system::Config>(swap_id: u64) -> DispatchResult {
    use crate::{SwapRequests, Pallet, Event, Error};
...
/// # 返回
/// - Result<u64, DispatchError>: 成功返回兑换ID
pub fn do_maker_swap<T: Config + frame_system::Config>(
    user: &T::AccountId,
    maker_id: u64,
    dust_amount: BalanceOf<T>,
    usdt_address: Vec<u8>,
) -> Result<u64, DispatchError> {
    use crate::{NextSwapId, MakerSwaps, MakerApplications, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_mark_swap_complete<T: Config + frame_system::Config>(
    maker: &T::AccountId,
    swap_id: u64,
    trc20_tx_hash: Vec<u8>,
) -> DispatchResult {
    use crate::{MakerSwaps, Pallet, Event, Error};
...
/// # 返回
/// - DispatchResult
pub fn do_report_swap<T: Config + frame_system::Config>(
    user: &T::AccountId,
    swap_id: u64,
) -> DispatchResult {
    use crate::{MakerSwaps, Pallet, Event, Error};
